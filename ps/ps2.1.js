...
PS.DrawBead = function (bead, ctx)
{
	...
	if ( ctx === undefined )
	{
		PS.Oops("No context passed in! Bad!");
		//ctx = PS.Context();
	}
...
/*
// Optimized function to copy beads from one spot to another
PS.BeadCopy = function (xDest, yDest, xSrc, ySrc)
{
	"use strict";
	var iSrc = xSrc + (ySrc * PS.Grid.x);		// Index of source bead
	var iDest = xDest + (yDest * PS.Grid.x);	// Index of destination bead
	var beadSrc = PS.Grid.beads[iSrc];
	var beadDest = PS.Grid.beads[iDest];
	
	beadDest.top = beadSrc.top;
	beadDest.left = beadDest.left;
		
	PS.DoBeadColor(xDest, yDest, beadSrc.color);
	PS.DoBeadBorderColor(xDest, yDest, beadSrc.borderColor);
	PS.DoBeadGlyph(xDest, yDest, beadSrc.glyph);
	PS.DoBeadGlyphColor(xDest, yDest, beadSrc.glyphColor);
	PS.DoBeadData(xDest, yDest, beadSrc.data);
};

PS.RowShift = function (numRows)
{
	var xAmount = numRows * PS.Grid.x;
	for(var i = 0; i < xAmount; +=i)
	{
		//fruits.unshift( PS.InitBead());
	}
}
*/
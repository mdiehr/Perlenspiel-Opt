...
( function ()
{
	...
	*/
	...
	// [types] is a fileTypes array (or null)
...
//		_buffers.push( buffer ); // save buffer object now?
			...
			_context.decodeAudioData( request.response,
			                          function ( audio ) // on successful load
			                          {
				                          ...
				                          if ( exec )
				                          {
					                          ...
					                          buffer.params.onLoad = null; // make sure it doesn't get called again!
					                          try
					                          {
						                          exec( buffer.params.data );
					                          }
					                          catch ( err )
					                          {
						                          _errorCatch( fn + ".onLoad function failed @ " + params.pathname + " [" + err.message + "]", err );
					                          }
				                          }
	...
	function _onErrorHTML5 ( event )
	{
		...
		return _error( "HTML5 Audio error " + c + " [" + str + "] : " + channel.params.pathname );
	}
	...
	function _onStalledHTML5 ()
	{
		...
		return _error( "Channel " + channel.id + " stalled: " + channel.params.pathname );
	}
	...
	function _onLoadHTML5 ()
	{
		...
		if ( exec )
		{
			...
			{
				_errorCatch( fn + "XMLHttpRequest failed @ " + params.pathname + " [" + err.message + "]", err );
				return;
			}
		}
	...
	function _onDoneHTML5 ()
	{
		...
		exec = params.onDone;
		if ( exec ) // one more check
		{
			try
			{
				exec( params.data ); // call it with user data
			}
			catch ( err )
			{
	...
	AQ = {
		...
		init : function ( params )
		{
			...
			if ( params )
			{
				...
				if ( type !== "undefined" )
				{
					...
					{
						window.alert( fn + ".forceHTML5 property not a boolean" );
						return AQ.ERROR;
					}
					if ( _monitor )
					{
		...
		load : function ( user_name, user_params )
		{
			...
			if ( _monitor )
			{
				_debug( fn + "Loaded " + user_name + ", result = " + result );
			}
		...
		play : function ( user_name, user_params )
		{
			...
			channel_id = _playExec( params );

			if ( _monitor )
			{
				_debug( fn + "Played " + user_name + ", channel = " + channel_id );
			}

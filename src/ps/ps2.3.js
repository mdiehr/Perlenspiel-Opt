...
var PS = {
	...
	CTX_CACHE: null,				// Cache of the main PS context
	MAIN_ID: "mn",
	STS_ID: "sts",
	CVS_ID: "cvs",
	FTR_ID: "ftr",
	...
	FONT_WEB: "ArialAndSymbola",
	FONT_DEFAULT: "sans-serif",
	FONT: "sans-serif",
	USE_WEB_FONT: false,
	
	// Debugger options
	DEBUG_STACK_ENGINE: false,	// If true, you see the whole debug stack including the game engine. Otherwise you just see game.js
	DEBUG_STACK_HTML: false,	// If true, you see the .html source in all errors (normally not useful)
...
//	ChromeBufferCnt: 0, // number of opened buffers
	...
	// Name slots for user callback functions
...
PS.Context = function ()
{
	...
	// If a cache of the context exists, fetch it
	if( PS.CTX_CACHE !== null )
	{
		return PS.CTX_CACHE;
	}
	else
	{
		// The canvas/context Didn't exist, so create it
		var cv, ctx;
...
PS.ValidRGB = function ( rgb, fn )
{
	...
	if ( typeof rgb !== "number" || isNaN(rgb) )
	{
	...
	if ( rgb < 0 )
	{
		PS.OopsCallstack( fn + "rgb parameter negative" );
		return -1;
	}
	if ( rgb > 0xFFFFFF )
	{
		PS.OopsCallstack( fn + "rgb parameter out of range" );
		return -1;
	}
	return rgb;
};
...
PS.UnmakeRGB = function ( rgb )
{
	...
	if ( rgb < 0 )
	{
		return PS.OopsCallstack(fn + "RGB parameter negative");
	}
	if ( rgb > 0xFFFFFF )
	{
		return PS.OopsCallstack(fn + "RGB parameter out of range");
	}
...
PS.ColorParams = function (fn, rgb, g, b)
{
	...
	if ( type === "number" || isNaN(rgb) )
	{
		if ( g === undefined ) // assume a multiplex
		{
			if ( (rgb < 0) || (rgb > 0xFFFFFF) )
			{
				PS.OopsCallstack(fn + "multiplexed rgb value out of range");
				return null;
			}
			rgb = Math.floor(rgb);
		...
		else // assume a triplet
		{
			...
			if ( blue === undefined )
			{
				PS.OopsCallstack(fn + "b parameter missing in rgb triplet");
				return null;		
			}	
		}
	}	
	...
	else if ( type === "object" )
	{
		...
		if ( red === undefined )
		{
			PS.OopsCallstack(fn + "r element missing in rgb table");
			return null;		
		}
		green = rgb.g;
		if ( green === undefined )
		{
			PS.OopsCallstack(fn + "g element missing in rgb table");
			return null;		
		}
		blue = rgb.b;
		if ( blue === undefined )
		{
			PS.OopsCallstack(fn + "b element missing in rgb table");
			return null;		
		}
	}
	...
	else if ( type === "array" )
	{
		if ( rgb.length < 3 )
		{
			PS.OopsCallstack(fn + "rgb array length invalid");
			return null;		
		}
		red = rgb[0];
		green = rgb[1];
	...
	{
		PS.OopsCallstack(fn + "Invalid color parameter");
		return null;
	}	
	...
	if ( !valid )
	{
		...
		if ( (red < 0) && (red > 255) )
		{
			PS.OopsCallstack(fn + "red value out of range [" + red + "]");
			return null;
		}
		...
		{
			PS.OopsCallstack(fn + "green value out of range [" + green + "]");
			return null;
		}
		...
		if ( (blue < 0) && (blue > 255) )
		{
			PS.OopsCallstack(fn + "blue value out of range [" + blue + "]");
			return null;
		}
	}
...
PS.InitBead = function (xpos, ypos, size, bgcolor)
{
	...
	// Sets default properties for the bead
	PS.ResetBead(bead);

	return bead;
};

// Sets all of the properties in a bead to their default value
PS.ResetBead = function (bead)
{
	bead.dirty = true;		// bead needs to be redrawn?
...
PS.RenderBead = function(bead)
{
	"use strict";

	bead.dirty = false;
	
	var ctx = PS.Grid.context;
	var left = bead.left;
	var top = bead.top;
	var sizew = bead.size;	
	var sizeh = bead.size;
	...
	var width = bead.borderWidth;
	if ( width.length === 4 )	// Bead border array mode
	{
		ctx.fillStyle = bead.borderColor;
		ctx.fillRect(bead.left, bead.top, sizew, sizeh);
	...
	// Draw background color (smaller if there was a border)
	ctx.fillRect(left, top, sizew, sizeh);

	// Write glyph
	if ( bead.glyph > 0 )
	{
		ctx.fillStyle = bead.glyphColor;
		ctx.fillText (bead.glyphStr, bead.left + PS.Grid.glyphX, bead.top + PS.Grid.glyphY);
	}
...
PS.InitGrid = function (x, y)
{
	...
	grid.glyphSize = Math.floor(size / 2) + "pt sans-serif";
	...
	// set up font info for this context
	grid.context.font = Math.floor(size / 2) + "pt " + PS.FONT;
...
PS.CheckX = function ( x, fn )
{
	...
	// First check the grid to see if it's been set up
	if( PS.Grid === null )
	{
		PS.OopsCallstack(fn + "The grid hasn't been initialized yet. Make sure you call PS.GridSize before any other bead functions.");
		return false;
	}
	...
	if ( x < 0 )
	{
		PS.OopsCallstack(fn + "x parameter negative");
		return false;
	}
	if ( x >= PS.Grid.x )
	{
		PS.OopsCallstack(fn + "x parameter exceeds grid width");
		return false;
	}
	return true;
};
...
PS.CheckY = function ( y, fn )
{
	...
	if ( y < 0 )
	{
		PS.OopsCallstack(fn + "y parameter negative");
		return false;
	}
	if ( y >= PS.Grid.y )
	{
		PS.OopsCallstack(fn + "y parameter exceeds grid height");
		return false;
	}
	return true;
};
...
PS.GridSize = function (w, h)
{
	...
	else 
	{
		...
		if ( w < 1 )
		{
			PS.OopsCallstack(fn + "Width parameter < 1");
			w = 1;
		}
		else if ( w > PS.G_MAX )
		{
			PS.OopsCallstack(fn + "Width parameter > " + PS.G_MAX);
			w = PS.G_MAX;
		}
	}
	...
	else
	{
		...
		if ( h < 1 )
		{
			PS.OopsCallstack(fn + "Height parameter < 1");
			h = 1;
		}
		else if ( h > PS.G_MAX )
		{
			PS.OopsCallstack(fn + "Height parameter > " + PS.G_MAX);
			h = PS.G_MAX;
		}
	}

	// If a grid with the same size as an existing grid is in place, just erase it
	// This is faster than reinitializing the grid
	if( PS.Grid !== null && PS.Grid.x === w && PS.Grid.y === h )
	{
		PS.ResetGrid();
	}
	else
	{
		...
		if ( !PS.Grid )
		{
			return PS.OopsCallstack(fn + "Grid initialization failed");
		}
	}
...
PS.ClearCanvas = function(canvas)
{
	if ( canvas )
	{
		PS.Grid.context.clearRect( 0, 0, canvas.width, canvas.height );
	}
}

// Reset all beads in the grid to their default state
PS.ResetGrid = function ()
{
	if ( PS.Grid )
	{
		for( var i = 0; i < PS.Grid.beads.length; ++i )
		{
			var bead = PS.Grid.beads[i];
			if( bead !== null )
			{
				PS.ResetBead(bead);
			}
...
PS.XBBWidth = function (x, y, width)
{
	...
	var i, bead;
	var widthClone;
	if( PS.TypeOf(width) === "array" )
	{
		widthClone = [width[0], width[1], width[2], width[3]];
	}
	else
	{
		widthClone = width;
	...
	if ( (widthClone === undefined) || (widthClone === PS.CURRENT) ) // if no width or PS.CURRENT, return current width
	{
		return bead.borderWidth;
	}

	bead.borderWidth = widthClone;
...
PS.BeadBorderWidth = function (x, y, width)
{
	...
	else if ( (width !== undefined) && (width !== PS.CURRENT) )
	{
		...
		else if( PS.TypeOf(width) === "array" && width.length === 4 )
		{
			for(var i = 0; i < 4; i++ )
			{
				width[i] = Math.floor(width[i]);
				if ( width[i] < 0 )
				{
					width[i] = 0;
				}
				else if ( width[i] > PS.Grid.borderMax )
				{
					width[i] = PS.Grid.borderMax;
				}
			}
		}
		else
		{
			return PS.OopsCallstack(fn + "width param is not a number or array[4]");
		}
	}
...
PS.XBGlyph = function (x, y, g)
{
	...
	bead.glyph = g;
	bead.glyphStr = String.fromCodePoint(g);	// Supports unicode outside the BMP
...
PS.BeadGlyph = function (x, y, g)
{
	...
	if ( type !== "undefined" )
	{
		if ( type === "string" )
		{
			if ( g.length < 1 )
			{
				return PS.OopsCallstack(fn + "glyph param is empty string");
			}
			g = g.charCodeAt(0); // use only first character
		}
		else if ( type === "number" && !isNaN(g) )
		{
			g = Math.floor(g);
			if ( g === PS.DEFAULT )
			{
		...
		{
			return PS.OopsCallstack(fn + "glyph param not a string or number");
		}
	}
...
PS.BeadAudio = function (x, y, audio, volume)
{
	...
	if ( (audio !== undefined) && (audio !== PS.CURRENT) )
	{
		...
		else
		{
			...
			{
				return PS.OopsCallstack(fn + "Audio param is not a string");
			}
			if ( audio.length < 1 )
			{
				audio = null;
...
PS.BeadFunction = function (x, y, exec)
{
	...
	if ( (exec !== undefined) || (exec !== PS.CURRENT) )
	{
		...
		{
			return PS.OopsCallstack(fn + "exec param not a valid function");
		}
	}
...
PS.XBTouch = function (x, y)
{
	...
	{
		PS.AudioPlay(bead.audio, bead.volume, null, PS.CURRENT, undefined);
	}
...
PS.StatusText = function (str)
{
	...
	if( PS.Grid === null )
	{
		PS.OopsCallstack(fn + "The grid hasn't been initialized yet. Make sure you call PS.GridSize before any other status functions.");
		return false;
	}
	...
	if ( type !== "undefined" )
	{
		if ( type !== "string" )
		{
			return PS.OopsCallstack(fn + "Parameter is not a valid string");
		}
		e = document.getElementById(PS.STS_ID);
		if ( e )
		{
...
PS.Oops = function (str)
{
	...
	PS.Debug( "ERROR: " + str + "\n" );	
	PS.AudioPlay("fx_uhoh", PS.DEFAULT, null, PS.DEFAULT, undefined);

	return PS.ERROR;
};

PS.OopsCallstack = function (str)
{
	...
	{
		return PS.OopsCatch(str, err)
	}
}

PS.OopsCatch = function (str, err)
{
	var returnVal = PS.Oops(str + " [" + err.message + "]");
	PS.Debug(PS.DecodeCallstack(err.stack) + "\n");
	return returnVal;
}

/*
Chrome
ERROR: PS.Init() failed [GRID_h is not defined]
ReferenceError: GRID_h is not defined
    at Object.PS.Init (file:///C:/Users/mdiehr//game.js:39:24)

IE
ERROR: PS.Init() failed ['GRID_h' is undefined]
ReferenceError: 'GRID_h' is undefined
   at Init (file:///C:/Users/mdiehr/game.js:39:2)

Firefox
ERROR: PS.Init() failed [GRID_h is not defined]
PS.Init@file:///C:/Users/mdiehr/game.js:39

Opera
ERROR: PS.Init() failed [Undefined variable: GRID_h]
<anonymous function: PS.Init>([arguments not available])@file://localhost/C:/Users/mdiehr/game.js:39

*/
PS.DecodeStackLine = function(str)
{
	var result = "";
	...
	if( str.search(".js") !== -1 )	// Code lines
	{
		...
		var fileName = str.substr(index).replace(/^[\s\(\)]+|[\s\(\)]+$/g, '');
	...
	else if( PS.DEBUG_STACK_HTML && str.search(".htm") !== -1 )	// HTML line
	{
		result += "\n" + str;
	}
	else	// Other line, don't include the message
	{
		//result += "\n" + str;
	}
...
PS.DecodeCallstack = function(str)
{
	...
	for( i in lines )
	{
		result += PS.DecodeStackLine( lines[i] );
	}
...
PS.Clock = function ( ticks )
{
	...
	if ( ticks !== undefined )
	{
		...
		{
			return PS.OopsCallstack(fn + "PS.Tick function undefined");
		}
		PS.UserDelay = 0;
		PS.UserClock = ticks;
	}
	...
	if ( len > 0 )
	{
		...
		else
		{
			...
			while ( i < len )
			{
				...
				{
					PS.OopsCatch("Key down repeat failed", err);
				}
		...
		if ( PS.UserDelay >= PS.UserClock )
		{
			...
			if ( PS.Tick )
			{
				...
				{					
					...
					PS.OopsCatch("PS.Tick() failed", err2);
				}
...
PS.MouseXY = function (event)
{
	...
	if ( PS.Grid )
	{
	    canvas = document.getElementById(PS.CVS_ID);
...
PS.MouseDown = function (event)
{
	...
	if ( PS.MouseX >= 0 )
	{
		...
		{			
			...
			{
				PS.OopsCatch("Bead " + PS.MouseX + ", " + PS.MouseY + " function failed", err1);
			}			
		}
		...
		{			
			...
			{
				PS.OopsCatch("PS.Click() failed", err2);
			}	
		}
	}
};
...
PS.MouseUp = function (event)
{
	...
	if ( PS.Grid && PS.Release ) // only if grid and function exist
	{
		...
		if ( PS.MouseX >= 0 )
		{
			...
			{
				PS.OopsCatch("PS.Release() failed", err);
			}
		}
	}
};
...
PS.MouseMove = function (event)
{
		...
		if ( (PS.MouseX !== PS.LastX) || (PS.MouseY !== PS.LastY) )
		{
			if ( PS.Leave ) // only if function exists
			{
				if ( PS.LastX >= 0 )
				{
					...
					{
						PS.OopsCatch("PS.Leave() failed", err1);
					}					
				}
			}
			...
			{			
				...
				{
					PS.OopsCatch("PS.Enter() failed", err2);
				}				
			}
			PS.LastX = PS.MouseX;
			PS.LastY = PS.MouseY;
	...
	else if ( PS.LastX >= 0 )
	{
		if ( PS.Leave ) // only if function exists
		{
			...
			{
				PS.OopsCatch("PS.Leave() failed", err3);
			}			
		}
		PS.LastX = -1;
		PS.LastY = -1;
	...
	{		
		...
		if ( PS.LegalKey(key) )
		{
			...
			if ( !PS.Pressed[key] ) // if not already pressed ...
			{
				...
				if ( PS.Holding.length < 1 )
				{
					PS.KeyDelay = PS.KEY_RATE * 5; // set initial repeat delay if no other keys down
				}
	
				if ( PS.Holding.indexOf(key) < 0 ) // bug fixed by Mark Diehr
				{
				...
				{
					PS.OopsCatch("PS.KeyDown() failed", err);
				}
			}
		}
	}	
...
PS.SysKeyUp = function (event)
{
	...
	if ( PS.KeyUp ) // only if function exists
	{
		...
		if ( PS.LegalKey(key) )
		{
			...
			if ( PS.Holding.length < 1 ) // if no other keys held ...
			{
				PS.KeyDelay = 0; // stop repeats
				PS.HoldShift = 0;
				PS.HoldCtrl = 0;
			}
			...
			{
				PS.OopsCatch("PS.KeyUp() failed", err);
			}
		}	
	}	
	return false;
...
PS.SysWheel = function (event)
{
	...
	if ( PS.Wheel ) // only if function exists
	{
		...
		{
			PS.OopsCatch("PS.Wheel() failed", err);
		}
	}
...
PS.DoImageLoad = function ()
{
	...
	if ( imgdata !== PS.ERROR )
	{
		...
		{
			PS.OopsCatch("[PS.ImageLoad] Load function failed", err);
		}
	}
};
...
PS.ImageLoad = function ( file, func, format )
{
	...
	{
		return PS.OopsCallstack(fn + "Invalid file parameter");
	}
	
	if ( typeof func !== "function" )
	{
		return PS.OopsCallstack(fn + "Invalid function parameter");		
	}	
	...
	else
	{
		...
		{
			return PS.OopsCallstack(fn + "format is not 1, 3 or 4");
		}
	}
	...
	{
		return PS.OopsCatch("" + fn + "Load failed: " + file, err);
	}
...
PS.ImageData = function ( img, format )
{
	...
	{
		return PS.OopsCatch("" + fn + "Data extraction failed @ 1 " + file, err);
	}	
	...
	{
		return PS.OopsCatch("" + fn + "Data extraction failed @ 2 " + file, err2);
	}
...
PS.ImageBlit = function ( imgdata, xpos, ypos, left, top, width, height )
{
	...
	{
		return PS.OopsCallstack(fn + "Invalid imgdata parameter");
	}
	...
	if ( w < 1 )
	{
		return PS.OopsCallstack(fn + "imgdata.width < 1");	
	}
	...
	if ( h < 1 )
	{
		return PS.OopsCallstack(fn + "imgdata.height < 1");		
	}
	
	a = PS.D_ALPHA; // assume default alpha
	psize = imgdata.pixelSize;
	if ( typeof psize !== "number" || isNaN(psize) )
	{
	...
	{
		return PS.OopsCallstack(fn + "imgdata.pixelSize is not 1, 3 or 4");				
	}
	...
	if ( data.length !== t )
	{
		return PS.OopsCallstack(fn + "imgdata.data length invalid [" + imgdata.data.length + "]");		
	}
	...
	else
	{
		...
		if ( ( left < 0 ) || ( left >= w ) ) 
		{
			return PS.OopsCallstack(fn + "left parameter outside of image");	
		}
		...
		if ( ( top < 0 ) || ( top >= h ) ) 
		{
			return PS.OopsCallstack(fn + "top parameter outside of image");	
		}
		...
		if ( (left + width) > w ) 
		{
			return PS.OopsCallstack(fn + "left + width exceeds image width");	
		}
		...
		if ( (top + height) > h ) 
		{
			return PS.OopsCallstack(fn + "top + height exceeds image height");	
		}
	}
	...
	ctx.font = PS.Grid.glyphSize; // reset glyph size? MDiehr-NOTE: This line doesn't make sense
...
//					bead.alphaBlue = PS.Dissolve( PS.Grid.bgBlue, bead.alphaBlue, a );
					...
					// modification suggested by Mark Diehr
					
					bead.alphaRed = PS.Dissolve( bead.red, r, a );
					bead.alphaGreen = PS.Dissolve( bead.green, g, a );
					bead.alphaBlue = PS.Dissolve( bead.blue, b, a );
					
					bead.color = "rgb(" + bead.alphaRed + "," + bead.alphaGreen + "," + bead.alphaBlue + ")";
					
					// new code suggested by Mark Diehr
...
PS.AudioError = function (event)
{
	...
	return PS.OopsCallstack("HTML5 audio error " + c + " (" + str + ") '" + src + "'" );
};
...
//		PS.Debug("Load/playing sound in in " + i + "\n");
		PS.PlayHTML5Audio(i, undefined, undefined, undefined);
	}
};
...
//	PS.Debug("Ending " + i + "\n");
	...
	if ( func && (typeof func === "function") ) // one more check
	{
		try
		{
			...
			func(channel.data); // call it with user data
		}
		catch (err)
		{
			PS.OopsCatch("Audio function error" + file, err);
		}
	}	
};
...
PS.AudioInit = function ()
{
	...
	{
		PS.OopsCallstack(fn + "HTML5 audio not supported");
		return false;
	}
...
//	}
	...
	if ( !PS.AudioContext )
	{
		...
		for ( i = 0; i < PS.AUDIO_MAX_CH; i += 1 )
		{
			...
			if ( !snd )
			{
				return PS.OopsCallstack(fn + "Audio element init failed");
			}
			snd.setAttribute("data-channel", i ); // remember this element's channel
//			snd.addEventListener("error", PS.AudioError, false );
//			snd.addEventListener("canplaythrough", PS.AudioLoaded, false );
//			snd.addEventListener("ended", PS.AudioEnding, false );
//			snd.setAttribute("preload", "auto");						
			...
			PS.AudioChannels[i] = {
				...
				playNow: false,
				exec: null,
				num: i + 1, // 1-based channel number
				data: i + 1, // channel number is default data
				status: PS.CH_EMPTY
			};
		}
	}
...
PS.AudioPath = function (path)
{
	...
	if ( (path === undefined) || (path === PS.CURRENT) )
	{
	...
	{
		return PS.OopsCallstack(fn + "parameter not a valid string");
	}
...
PS.AudioLoad = function (id, path)
{
	...
	{
		return PS.OopsCallstack(fn + "ID parameter not a valid string");		
	}
	...
	else if ( typeof path !== "string" ) // empty string is a valid path!
	{
		return PS.OopsCallstack(fn + "Path parameter not a valid string");		
	}
	...
	if ( !PS.AudioContext )
	{
		...
		i = PS.LoadHTML5Audio(fullpath, undefined, undefined, undefined, undefined);
		if ( i === PS.ERROR )
		{
			return i;
		}
...
PS.AudioPlay = function (id, volume, func, path, data)
{
	"use strict";
...
//	var len, bobj;
	...
	if ( !func || (func === PS.DEFAULT) )
	{
		func = null;
	}
	else if ( typeof func !== "function" )
	{
		return PS.OopsCallstack(fn + "Func parameter not a valid function");			
	}	
...
//				PS.Debug("Playing sound in in " + i + "\n");
				return PS.PlayHTML5Audio (i, volume, func, data); // returns 1-based channel id
			}
		}
		
		// must load a new instance
		
		i = PS.LoadHTML5Audio(fullpath, volume, func, data, true); // load & play; returns 0-based channel index
		if ( i === PS.ERROR )
		{
			return i;
		}
...
//	var len, i;
	...
	if ( !PS.AudioContext )
	{
		...
		{
			return PS.OopsCallstack(fn + "Invalid channel id");		
		}
...
//	var	len, i, fullpath, buffer, volume, func, pauseTime;
	...
	{	
		if ( (c < 1) || (c > PS.AUDIO_MAX_CH) )
		{
			return PS.OopsCallstack(fn + "Invalid channel id [" + c + "]");			
		}
		...
		if ( audio.paused )
		{
			PS.PlayHTML5Audio(c - 1, undefined, undefined, undefined);
		}
		else
		{
...
PS.LoadChannel = function (i, fullpath, volume, func, data, now)
{
	"use strict";
	var channel, snd;
	...
	snd.addEventListener("ended", PS.AudioEnding, false);
	snd.preload = "auto";
	snd.volume = channel.volume;
	snd.setAttribute("src", fullpath); // actually calls load
					
	channel.status = PS.CH_LOADING;
};

PS.LoadHTML5Audio = function (fullpath, volume, func, data, now)
{
	"use strict";
	var fn, channel, i;
	...
	for ( i = 0; i < PS.ChannelsUsed; i += 1 )
	{
		...
		{			
			...
			if ( now )
			{
				PS.PlayHTML5Audio(i, undefined, undefined, undefined); // just play it again
			}
	...
	for ( i = 0; i < PS.AUDIO_MAX_CH; i += 1 ) // search all channels
	{
		...
		{			
			...
			PS.LoadChannel(i, fullpath, volume, func, data, now);	
//			err = channel.audio.error;
//			if ( err )
//			{
//				return PS.AudioError(err);
//			}
			return i;
		}
	}
...
PS.PlayHTML5Audio = function (i, volume, func, data)
{
	"use strict";
	var channel;	
...
PS.LoadChromeAudio = function (fullpath, volume, func, channel, offsetTime)
{
	...
	if ( !request )
	{
		return PS.OopsCallstack(fn + "XMLHttpRequest error");
	}
	...
	request.onload = function ()
	{
		PS.AudioContext.decodeAudioData ( request.response,
			...
			function () // on error
			{
				PS.OopsCallstack(fn + "Error loading " + fullpath + "]");
			} );
	};
	...
	{		
		return PS.OopsCallstack(fn + "XMLHttpRequest failed: " + fullpath + " [" + err.message + "]");
	}
...
PS.PlayChromeAudio = function (fullpath, buffer, volume, func, channel, offsetTime)
{
	...
	if ( !source )
	{
		return PS.OopsCallstack(fn + "Invalid bufferSource");
	}
	...
	if ( !gainNode )
	{
		return PS.OopsCallstack(fn + "Invalid gainNode");
	}
...
PS.StyleElementForFonts = function(element, id)
{
	element.id = id;
...
PS.CheckLoadingProgress = function()
{
	PS.fontLoaderW = PS.fontLoader.scrollWidth;
	
	var diff = Math.abs(PS.fontBaseW - PS.fontLoaderW);
	PS.FontTicks--;
	
	if( diff > 10 || PS.FontTicks <= 0 )
	{
		clearInterval(PS.FontInterval);
		document.body.removeChild(PS.fontLoader);
		document.body.removeChild(PS.fontBase);
		PS.LoadEngine();
	}
}

// Font Initialization
PS.Sys = function ( useWebFont )
{
	if( useWebFont === true )
	{
		PS.USE_WEB_FONT = true;
	}

	if( PS.USE_WEB_FONT === true )
	{
		PS.FONT = PS.FONT_WEB;
		
		PS.fontBase = document.createElement("div");
		PS.StyleElementForFonts(PS.fontBase, "FontBase");
		
		PS.fontLoader = document.createElement("div");
		PS.fontLoader.style.fontFamily = PS.FONT;
		PS.StyleElementForFonts(PS.fontLoader, "FontLoader");
		
		PS.fontBaseW = PS.fontBase.scrollWidth;
		
		PS.FontInterval = setInterval(PS.CheckLoadingProgress, 10);
		PS.FontTicks = 300;
	}
	else
	{
		PS.FONT = PS.FONT_DEFAULT;
		PS.LoadEngine();
	}
}
...
//	e.value = PS.Status;
	...
	PS.AudioLoad("fx_uhoh", PS.DEFAULT);
	...
	PS.BlitContext.textAlign = "center";
	PS.BlitContext.textBaseline = "middle";
	...
	if ( PS.Init )
	{
		...
		{
			PS.OopsCatch("PS.Init() failed", err);
		}		
	}
	else
	{
